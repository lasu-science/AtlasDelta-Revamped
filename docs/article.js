...
// ahora con lectura async de Firestore para los artículos)
function LibraryArticle() {
  ...
  Promise.all([getArticle(slug), getAllArticles()]).then(function(results){
    var article = results[0], ALL = results[1];
    root.innerHTML = '';
    if (!article) { root.appendChild(NotFound()); return; }
